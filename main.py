...
def init_parser():
    ...
    # Master's Project Data specific args
    parser.add_argument('--ind_class_ratio', default=1., type=float, help='The percentage of training classes to be in-distribution.')
    parser.add_argument('--ind_example_ratio', default=1., type=float, help='The percentage of in-distribution examples to use to train.')
    ...
    parser.add_argument('--ra', default=3, type=int, help='repeated augmentation')
...
def main(args):
    ...
    # Set GPU given by args as main GPU
    if not args.no_cuda and torch.cuda.is_available():
        if args.gpu < torch.cuda.device_count():
            torch.cuda.set_device(args.gpu)
        else:
            raise ValueError(f"GPU {args.gpu} not available. Only {torch.cuda.device_count()} GPUs found.")

    data_info = datainfo(logger, args)

    model = create_model(data_info['img_size'], int(data_info['n_classes'] * args.ind_class_ratio), args)

    model.cuda(args.gpu) # move model to specific device
    ...
    train_dataset, val_dataset, class_info = dataload(args, augmentations, normalize, data_info)
